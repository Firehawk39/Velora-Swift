    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                // ── Greeting ─────────────────────────────────────────
                Text(greeting)
                    .font(.system(size: UIScaler.scaleFont(isCompact ? 26 : 28), weight: .bold))
                ...
                if !offlineRecent.isEmpty || network.isConnected {
                    ...
                    if client.recentTracks.isEmpty {
                        SkeletonRow(count: 4, cardWidth: UIScaler.scaleW(isCompact ? 140 : 160), cardHeight: UIScaler.scaleW(isCompact ? 140 : 160), isDark: isDark)
                    } else {
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: UIScaler.scaleW(isCompact ? 16 : 24)) {
                                ForEach(offlineRecent.prefix(isCompact ? 4 : 6)) { track in
                                    TrackCard(
                                        track: track,
                                        isDark: isDark,
                                        size: UIScaler.scaleW(isCompact ? 140 : 160),
                ...
                if !offlineArtists.isEmpty || network.isConnected {
                    ...
                    if client.artists.isEmpty {
                        SkeletonRow(count: 5, cardWidth: UIScaler.scaleW(isCompact ? 75 : 90), cardHeight: UIScaler.scaleW(isCompact ? 75 : 90), isDark: isDark, circular: true)
                    } else {
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: UIScaler.scaleW(isCompact ? 16 : 24)) {
                                ForEach(offlineArtists.prefix(isCompact ? 8 : 12)) { artist in
                                    ArtistCircle(artist: artist, isDark: isDark, size: UIScaler.scaleW(isCompact ? 75 : 90))
                                        .onTapGesture {
                ...
                if !offlineAlbums.isEmpty || network.isConnected {
                    ...
                    if client.albums.isEmpty {
                        SkeletonRow(count: 3, cardWidth: UIScaler.scaleW(isCompact ? 160 : 200), cardHeight: UIScaler.scaleH(isCompact ? 100 : 130), isDark: isDark, rounded: 24)
                    } else {
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: UIScaler.scaleW(isCompact ? 12 : 24)) {
                                ForEach(offlineAlbums.prefix(isCompact ? 6 : 8)) { album in
                                    AlbumCard(album: album, isDark: isDark, cardW: UIScaler.scaleW(isCompact ? 160 : 200), cardH: UIScaler.scaleH(isCompact ? 100 : 130))
                                        .onTapGesture {
